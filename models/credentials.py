...
def get_plex_account(
    creds: Credentials = Depends(Credentials), timeout: int = 10
) -> MyPlexAccount:
...
def get_plex_server(
    creds: Credentials = Depends(Credentials), timeout: int = 4
) -> PlexServer:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Attempting to direct connect to Plex server without IP address.",
        )
    except Unauthorized:
        raise HTTPException(
            status_code=401, detail="Invalid Plex token. Please sign in again."
        )
    except requests.exceptions.ConnectionError:
        raise HTTPException(
            status_code=503,
            detail=f"Plex server connection timeout. IP: {creds.server_ip}",
        )
